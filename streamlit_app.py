...
# Reemplaza esto con tu URL real de Google Sheets
URL_HOJA = "AQUÍ_VA_TU_URL_COMPLETA_DE_GOOGLE_SHEETS"

# --- CONEXIÓN PURA CON BASE64 (LO QUE YA HABÍA FUNCIONADO) ---
try:
    # Leer el string de los Secrets
    b64_texto = st.secrets["gserviceaccount"]["base64_creds"]
    
    # Decodificar de Base64 a bytes y luego a texto plano de forma segura
    ...
    with st.form("form_aporte", clear_on_submit=True):
        ...
        if submit_aporte:
            if persona and monto_aporte > 0:
                ...
                # append_row no falla nunca, sin importar cómo esté la hoja
                ws_aportes = sh.worksheet("Aportes")
